...
<div x-data="{ 
...
}" class="bg-[#f4f6fa] text-slate-800 min-h-screen">
    ...
    <header class="sticky top-0 z-40 bg-white shadow-sm">
        <nav class="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
            ...
            <ul class="hidden lg:flex items-center gap-7 text-[13px] font-medium text-slate-600">
                ...
                <li class="flex items-center gap-1 cursor-pointer"><a href="#profil">Profil</a> <span class="text-[10px]">▾</span></li>
                <li class="cursor-pointer"><a href="#profil">Fasilitas KBM</a></li>
                <li class="flex items-center gap-1 cursor-pointer"><a href="#civitas">Digital Akademik</a> <span class="text-[10px]">▾</span></li>
            ...
            <a href="#spmb" class="bg-navy-900 text-white text-xs font-semibold px-5 py-2 rounded-sm hover:bg-navy-800 transition">SPMB</a>
        </nav>
    </header>

    <!-- ============ HERO (DYNAMIC BANNERS LOOP FROM DB) ============ -->
    <section class="relative hero-bg overflow-hidden min-h-[500px]" x-init="if (totalSlides > 1) { setInterval(() => { activeSlide = (activeSlide + 1) % totalSlides }, 6000) }">
        @if(count($banners) > 0)
            @foreach($banners as $index => $banner)
                <div x-show="activeSlide === {{ $index }}" x-transition:enter="transition ease-out duration-700" x-transition:enter-start="opacity-0 scale-105" x-transition:enter-end="opacity-100 scale-100" class="max-w-7xl mx-auto px-6 pt-14 pb-28 grid lg:grid-cols-2 gap-8 items-center relative z-10">
                    <div>
                        <h1 class="text-white text-4xl md:text-5xl font-bold leading-tight mb-4 font-sans">
                            {{ $banner->title }}
                        </h1>
                        <p class="text-slate-200/80 text-sm max-w-md mb-6 leading-relaxed">
                            <strong>Smada Prima</strong> — {{ $banner->description ?? 'Selamat datang di website resmi SMA Negeri 2 Situbondo — sekolah unggulan yang berkomitmen mencetak generasi berprestasi, berkarakter, dan siap bersaing di era global.' }}
                        </p>
    ...
    <section class="bg-navy-900 mt-14">
        <div class="max-w-6xl mx-auto px-6 py-12 relative">
            ...
            <div class="grid md:grid-cols-[220px_1fr] gap-8 items-start">
                <img src="/build/assets/kepala sekolah smada.png" class="rounded-md w-full h-72 object-cover border-2 border-white/20 shadow-lg" alt="Kepala Sekolah SMAN 2 Situbondo"/>
                <div>
                    <h3 class="text-white font-bold tracking-wide text-xl">NIKMATIL HASANAH, S.Pd, M.Pd</h3>
                    <p class="text-gold text-xs mb-4 font-semibold">19640516 200604 2 012</p>
    ...
    <!-- ============ BERITA SMADA (TOP 5 PUBLISHED DINAMIS FROM DATABASE) ============ -->
    <section class="max-w-6xl mx-auto px-6 py-12" id="berita">
        <div class="flex justify-between items-center mb-4">
            <h2 class="font-bold text-navy-900 text-lg">Berita Smada</h2>
            <a href="#berita" class="text-xs text-slate-400 hover:text-orange transition font-semibold">Selengkapnya »</a>
        </div>
        <div class="w-full h-px bg-slate-200 mb-6"></div>

        @if(count($newsList) > 0)
            <div class="grid md:grid-cols-3 gap-5">
                <!-- Main Featured Card (Left 1 Card) -->
                ...
                <div class="bg-white rounded-md overflow-hidden shadow-sm border border-slate-100 flex flex-col justify-between">
                    <div>
                        <img src="{{ $firstNews->thumbnail_url ?? '/build/assets/banner smada.png' }}" class="w-full h-44 object-cover" alt="{{ $firstNews->title }}"/>
                    ...
                    <div class="p-4 pt-0">
                        <a href="#berita" class="inline-block border border-navy-700 text-navy-700 hover:bg-navy-700 hover:text-white text-[11px] font-semibold px-3.5 py-1 rounded-full transition">Selengkapnya</a>
                    </div>
                </div>

                <!-- Small News Grid (Right 4 Side Cards) -->
        ...
        <div class="max-w-4xl mx-auto px-6 text-center">
            ...
            <div x-show="activeTab === 'siswa'" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="border border-gold/40 rounded-md py-6 bg-navy-950/40">
                    ...
                    <p class="text-slate-300 text-[11px] mt-1 uppercase font-semibold">Total Peserta Didik</p>
                </div>
                <div class="border border-gold/40 rounded-md py-6 bg-navy-950/40">
                    <p class="text-orange text-3xl font-extrabold">{{ $studentStats['kelas_10'] }}</p>
                    <p class="text-slate-300 text-[11px] mt-1 uppercase font-semibold">Siswa Kelas X</p>
                </div>
                <div class="border border-gold/40 rounded-md py-6 bg-navy-950/40">
                    <p class="text-orange text-3xl font-extrabold">{{ $studentStats['kelas_11'] + $studentStats['kelas_12'] }}</p>
                    <p class="text-slate-300 text-[11px] mt-1 uppercase font-semibold">Siswa Kelas XI &amp; XII</p>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'guru'" class="max-w-md mx-auto border border-gold/40 rounded-md py-8 bg-navy-950/40">
                ...
                <p class="text-slate-300 text-xs mt-1 uppercase font-semibold">Guru (Tenaga Pendidik)</p>
            </div>
            ...
            <div x-show="activeTab === 'staf'" class="max-w-md mx-auto border border-gold/40 rounded-md py-8 bg-navy-950/40">
                ...
                <p class="text-slate-300 text-xs mt-1 uppercase font-semibold">Staff (Tenaga Kependidikan)</p>
            </div>
        </div>
    </section>
    ...
    <section class="max-w-6xl mx-auto px-6 py-12 grid md:grid-cols-[2fr_1fr] gap-10" id="pengumuman">
        ...
        <div>
            <div class="flex justify-between items-center mb-3">
                ...
                <a href="#pengumuman" class="text-xs text-slate-400 hover:text-orange transition font-semibold">Selengkapnya »</a>
            </div>
