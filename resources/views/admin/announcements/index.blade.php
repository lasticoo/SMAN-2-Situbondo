...
<div class="flex min-h-screen bg-slate-50 text-slate-800 font-sans">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-slate-50">
        ...
        <header class="h-16 shrink-0 bg-white border-b border-slate-200 px-6 md:px-8 flex items-center justify-between sticky top-0 z-20 shadow-xs">
            <div>
                <h2 class="text-lg font-bold text-slate-800">Manajemen Pengumuman</h2>
                <p class="text-xs text-slate-500">Kelola pengumuman dan informasi resmi portal SMAN 2 Situbondo.</p>
            </div>
        ...
        <main class="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-slate-500 font-medium">Total Pengumuman</p>
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
                <form action="{{ route('admin.announcements.index') }}" method="GET" class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3 items-center">
                    ...
                    <div class="relative w-full sm:col-span-2 md:col-span-5">
                        ...
                        <input
                            ...
                            placeholder="Cari judul, ringkasan, atau konten pengumuman..."
                    ...
                    <table class="w-full text-left border-collapse min-w-[760px]">
                        <thead>
                            <tr class="bg-slate-50/80 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                                <th class="py-3.5 px-5 w-7/12">Pengumuman & Ringkasan</th>
                                ...
                                <tr class="hover:bg-slate-50/60 transition-colors group">
                                    ...
                                    <td class="py-4 px-5">
                                        <div class="flex items-start gap-4">
                                            <img
                                                src="{{ $announcement->display_thumbnail_url }}"
                                    ...
                                    <td class="py-4 px-4 whitespace-nowrap text-slate-600">
                                        ...
                                        <div class="text-[11px] text-slate-400">
                                            {{ $announcement->published_at ? $announcement->published_at->format('H:i') . ' WIB' : '-' }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-5 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <button
                                                onclick="openEditModal({{ json_encode($announcement) }})"
                                            ...
                                            <button
                                                onclick="openDeleteModal('{{ route('admin.announcements.destroy', $announcement) }}', '{{ addslashes($announcement->title) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-slate-400">
                                        ...
                                        <p class="text-xs font-semibold text-slate-500">Belum ada pengumuman yang ditemukan.</p>
...
<div id="createModal" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white w-full max-w-2xl rounded-2xl shadow-xl border border-slate-100 overflow-hidden transform transition-all my-8">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
                <h3 class="font-bold text-slate-800 text-sm">Tambah Pengumuman Baru</h3>
        ...
        <form action="{{ route('admin.announcements.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    required
                    maxlength="200"
                    placeholder="Contoh: Jadwal Pelaksanaan Ujian Semester Ganjil 2026/2027"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">
                        Thumbnail (.webp / JPG / PNG) <span class="text-rose-500">*</span>
                    </label>
                    <input
                        type="file"
                        name="thumbnail"
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">
                    Ringkasan Pengumuman <span class="text-rose-500">*</span>
                </label>
                <textarea
                    name="summary"
                    rows="2"
                    required
                    maxlength="300"
                    placeholder="Tuliskan ringkasan singkat pengumuman (maksimal 300 karakter)..."
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">
                    Konten Pengumuman Lengkap <span class="text-rose-500">*</span>
                </label>
                <textarea
                    name="content"
                    rows="5"
                    required
                    placeholder="Tuliskan seluruh rincian pengumuman di sini..."
...
<div id="editModal" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white w-full max-w-2xl rounded-2xl shadow-xl border border-slate-100 overflow-hidden transform transition-all my-8">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
                <h3 class="font-bold text-slate-800 text-sm">Edit Data Pengumuman</h3>
                <p class="text-[11px] text-slate-500">Perbarui rincian pengumuman di bawah ini.</p>
            </div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 p-6 text-center transform transition-all">
        ...
        <h3 class="text-base font-bold text-slate-900 mb-1">Konfirmasi Hapus Pengumuman</h3>
        <p class="text-xs text-slate-500 mb-4">
            Apakah Anda yakin ingin menghapus pengumuman <strong id="deleteTargetTitle" class="text-slate-800"></strong>? File thumbnail di penyimpanan juga akan dibersihkan.
        </p>
...
<script>
    ...
    function openEditModal(announcement) {
        ...
        document.getElementById('edit_category').value = announcement.category || 'Umum';
        document.getElementById('edit_status').value = announcement.status || 'published';
        document.getElementById('edit_summary').value = announcement.summary || '';
        ...
        if (announcement.display_thumbnail_url) {
            document.getElementById('edit_thumbnail_preview').src = announcement.display_thumbnail_url;
