...
<aside class="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 sticky top-0 h-screen z-30">
    ...
    <nav class="flex-1 px-4 py-5 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <!-- Kelola SPMB -->
        <a href="{{ Route::has('admin.spmb_info.index') ? route('admin.spmb_info.index') : url('/admin/spmb-info') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all {{ request()->routeIs('admin.spmb_info.*') || request()->is('admin/spmb*') ? 'font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 shadow-xs' : 'font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 {{ request()->routeIs('admin.spmb_info.*') || request()->is('admin/spmb*') ? 'text-indigo-600' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <a href="{{ Route::has('admin.admin_accounts.index') ? route('admin.admin_accounts.index') : url('/admin/admin-accounts') }}" class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all {{ request()->routeIs('admin.admin_accounts.*') || request()->is('admin/admin-accounts*') ? 'font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 shadow-xs' : 'font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 {{ request()->routeIs('admin.admin_accounts.*') || request()->is('admin/admin-accounts*') ? 'text-indigo-600' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
            </svg>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between px-2">
            <div class="flex items-center gap-3 overflow-hidden">
                @if(!empty($currentAdmin?->avatar_url))
                    <img src="{{ Storage::url($currentAdmin->avatar_url) }}" alt="{{ $currentAdmin->name ?? 'Admin' }}" class="w-8 h-8 rounded-full object-cover shrink-0 shadow-sm border border-slate-200">
                @else
                ...
                <div class="overflow-hidden">
                    ...
                    <p class="text-[10px] text-slate-500 truncate">
                        Role: {{ $currentAdmin?->role === 'super_admin' ? 'Super Admin' : ucwords(str_replace('_', ' ', $currentAdmin?->role ?? 'Admin')) }}
                    </p>
                </div>
            </div>
